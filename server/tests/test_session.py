...
# In-memory store to simulate DB for tests
_fake_store: dict = {}
...
@pytest.fixture
def mock_db():
    """Patch DB functions to use in-memory store."""
    with (
...
async def test_create_session_stores_user_and_tokens(mock_db):
    """create_session stores user and encrypted refresh token."""
    data = {
...
async def test_delete_session_removes_from_db(mock_db):
    ...
    data = {"user": {}, "tokens": {"access_token": "at", "refresh_token": "rt"}}
    session_id = await create_session(data)
    assert session_id in _fake_store
